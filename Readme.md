# Disclaimer:
**Dieses Projekt ist ein vollständig funktionsfähiges und einsatzbereites Werkzeug, das ausschließlich für Forschungs-, Analyse- und Testzwecke im Bereich der IT-Sicherheit bereitgestellt wird. Jegliche Nutzung, Verbreitung oder Anwendung auf fremden Systemen ohne die ausdrückliche und schriftliche Erlaubnis des Eigentümers ist strengstens untersagt und kann straf- sowie zivilrechtliche Konsequenzen nach sich ziehen. Der Autor übernimmt keinerlei Haftung für Schäden, Datenverluste oder sonstige Folgen, die durch unsachgemäßen, fahrlässigen oder missbräuchlichen Einsatz dieses Projekts entstehen. Verwenden Sie dieses Projekt niemals auf produktiven Systemen oder außerhalb von kontrollierten, legalen Testumgebungen. Die Verantwortung für die Einhaltung aller geltenden Gesetze und Vorschriften liegt ausschließlich beim Nutzer.**
...
**Konfigurationen:**
    ...
    FILEPATH = "key.txt" # Universeller verschlüsselter AES-Schlüssel
    ...
    ENCRYPTED_KEY_FILENAME = "key.txt"        # Name der Datei für den universellen verschlüsselten AES-Schlüssel
...
- `AresLocker/encryptor/arciv/`
- `AresLocker/keys/` (Schlüsseldateien: `private.pem`, `public.pem`)
- `AresLocker/decryptor/`

**Prozesse:**
- `note.exe`

**Registry-Keys (Persistenz):**
- `HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run` (Autostart-Eintrag)
...
**Weitere Hinweise:**
- Dateien und Prozesse, die im Zusammenhang mit den oben genannten Namen stehen
- Unerwartete Dateien im Autostart-Ordner oder geplante Tasks
- Logdateien oder Schlüsseldateien im Benutzerverzeichnis

Diese Liste ist nicht abschließend und sollte ggf. um weitere, im Einsatz beobachtete IOCs ergänzt werden.
